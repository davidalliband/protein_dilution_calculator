...
TP_data = TP_data.reindex(columns=["Content", "Total_protein", "Total_protein_volume", "Dilution_factor", "Sample_protein_quantity", "Loading_dye_fraction", "Total_solution_ul", "Sample_volume_ul", "Buffer_volume_ul", "Loading_dye_volume_ul"])

# Tidy up dataframe by renaming columns
TP_data.set_axis(["Content", "Total protein", "Total protein volume (ul)", "Dilution factor", "Sample protein quantity (ug)", "Loading dye fraction", "Total solution (ul)", "Sample volume (ul)", "Buffer volume (ul)", "Loading dye volume (ul)"], axis='columns', inplace=True)
